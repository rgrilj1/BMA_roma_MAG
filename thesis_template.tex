...
Umestitev umetne inteligence v številnih področjih je postala ključna za inovacije in izboljšave v različnih industrijah. Na področju računalniškega vida (Computer Vision) AI modeli omogočajo prepoznavo objektov in obrazov, kar najde uporabo v varnostnih sistemih, avtonomnih vozilih in medicinski diagnostiki. Z uporabo AI v analizi zvoka (Audio) se razvijajo pametni zvočniki in aplikacije za prepoznavanje govora. Na področju tabularnih podatkov AI modeli optimizirajo poslovne procese in finančne napovedi. Okrepitveno učenje (Reinforcement Learning) pomaga pri razvoju avtonomnih sistemov in robotike. Večmodalni pristop (Multimodal) združuje različne vrste podatkov, kar najdemo v napravah za virtualno resničnost. Govorno prepoznavanje (Speech) se uporablja za asistentke, medtem ko odločitveni modeli (Decision Making) vodijo strategije v podjetjih. Analitika (Analytics) s pomočjo AI modelov omogoča boljše razumevanje podatkov. Temeljna raziskava v AI (Foundation) pa omogoča napredek na vseh teh področjih. Vendar pa se bomo v naši raziskavi osredotočili na določena področja, kot sta obdelava naravnega jezika (NLP) in zaznavanje objektov (Object detection), saj sta ključnega pomena za našo študijo.
...
\end{figure}
 \newpage
V tabeli so predstavljeni stroški uporabe posameznih storitev. Pri nekaterih je potrebno plačati le virtualna okolja, za Azure Cognitive Services pa plačamo glede na zahtevke. Pomembno je omeniti, da je v tabeli uporabljena cena za Hugging Face Transformers, katere lahko uporabljamo tudi lokalno. 
%----------------------------------------------------------------
% Poglavje (Opis funkciionalnosti) 5
%----------------------------------------------------------------
...
AWS SageMaker klic:
\begin{lstlisting}[ basicstyle=\tiny,frame=single]
comprehend.detect_entities(Text=sample_tweet, LanguageCode='en')
    \end{lstlisting}
   

Azure Cognitive Services klic:
\begin{lstlisting}[ basicstyle=\tiny,frame=single]
client.recognize_entities(documents = documents)[0]
    \end{lstlisting}
...
AWS SageMaker klic:
\begin{lstlisting}[ basicstyle=\tiny,frame=single]
comprehend.detect_sentiment(Text=d, LanguageCode='en'))
    \end{lstlisting}
   

Azure Cognitive Services klic:
\begin{lstlisting}[ basicstyle=\tiny,frame=single]
client.recognize_sentiment(documents, show_opinion_mining=True)
    \end{lstlisting}
...
Azure Cognitive Services klic:
\begin{lstlisting}[ basicstyle=\tiny,frame=single]
client.extract_key_phrases(documents = documents)[0]
    \end{lstlisting}
...
Google Vertex AI klic:
\begin{lstlisting}[ basicstyle=\tiny,frame=single]
text_classification_label_sample(
