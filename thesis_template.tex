...
Zaznavanje: Računalnik preučuje sliko ali video posnetek in identificira regije, kjer bi se lahko nahajali objekti.

Lokalizacija: Po tem, ko so bile regije prepoznane, algoritem določi omejitveno okvirje (bounding boxes), ki natančno označujejo položaje in mejne točke objektov na sliki.

Klasifikacija: Ko so objekti omejeni z omejitvenimi okviri, računalnik analizira vsebino znotraj teh okvirov in jih razvrsti v različne kategorije (npr. avto, pes, zgradba, itd.).

Sledenje: V video posnetkih je lahko zaželeno, da algoritem sledi objektom skozi različne kadre in tako beleži njihovo gibanje.
